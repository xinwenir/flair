### 以下是对这段关于平行于坐标轴的棱锥（Pyramid parallel to a coordinate axis）描述内容的详细解释：

### 基本定义
- **类型说明**：文中提到的 `PYX`、`PYY`、`PYZ` 分别代表平行于 `x` 轴、`y` 轴、`z` 轴的棱锥类型。例如 `PYX` 类型的棱锥，其形状特点是棱锥的轴线方向与 `x` 轴平行，`PYY` 和 `PYZ` 同理，只是各自对应的平行坐标轴不同。
